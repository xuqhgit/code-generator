...
bp = Blueprint('module', __name__, template_folder='template', static_folder='../modules',static_url_path ='/static', url_prefix='/module')
...
def generate():
    ...
    return {"zip":get_zip(module_code,args["account"]),"files":get_file_path_list(module_code,args["account"])}



def get_zip(module_code,account):
    return ("%s/%s/%s/%s.zip" % (web.workplace,account,module_code,account)).split(web.web_dir)[1]
def get_file_path_list(module_code,account):
    path = "%s/%s/%s/file" % (web.workplace,account,module_code)
    files = []
    for main_dir, sub_dir, file_name_list in os.walk(path):
        for filename in file_name_list:
            file_path = os.path.join(main_dir, filename)
            files.append(file_path.split(web.web_dir)[1])
    return files