...
def initial_upper(source_str):
    if source_str is not None:
        return source_str[0].upper() + source_str[1:]
    return source_str

def initial_lower(source_str):
    if source_str is not None:
        return source_str[0].lower() + source_str[1:]
    return source_str

env.filters['initial_upper'] = initial_upper
env.filters['initial_lower'] = initial_lower
template_globals = {}


def format_datetime(datetime=None, date_format='%Y-%m-%d %H:%M:%S'):
    if datetime is None or datetime.isspace() or datetime is '':
        datetime = time.localtime(time.time())
    return time.strftime(date_format, datetime)

template_globals['now'] = format_datetime
...
def render_template(code, template_name, context):
    ...
    t = env.get_template(path,globals=template_globals)
...
def get_last_module_code(module_code_arr, template_name):
    ...
    for code in reversed(module_code_arr):
        if code in module_map and key_module_template in module_map[code] and bool(
                module_map[code][key_module_template]):
